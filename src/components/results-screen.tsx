...
const chivoMonoFont = Chivo_Mono({
  subsets: ["latin"],
  weight: ["400", "700"],
  display: "swap",
...
type Source = { title: string; url: string; religion?: string };
...
export function ResultsScreen({
  ...
  const [modalOpen, setModalOpen] = useState<ReligionKey | null>(null);
  ...
  // Font wrappers
  const chivoMono = chivoMonoFont.className;
  const instrumentSans = instrumentSansFont.className;
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className={`min-h-screen px-6 md:px-12 py-8 bg-black ${instrumentSans}`}
    >
      <div className="max-w-4xl mx-auto">
        ...
        <motion.h1
          ...
          className={`text-4xl md:text-5xl font-light mb-12 text-white ${instrumentSans}`}
        >
          {title}
        </motion.h1>
        ...
        <div className="flex flex-col gap-8 mb-12">
          {religions.map((religion, idx) => {
            const section = sections[religion];
            ...
            return (
              <motion.div
                ...
                className={`cursor-pointer border-2 ${religionColors[religion].border} ${cardTransition ? religionColors[religion].bg : "bg-transparent"} p-8 shadow-lg hover:scale-[1.02] transition-transform relative overflow-hidden ${instrumentSans} border-solid ${isError ? "bg-red-900/80" : ""}`}
                style={{ borderRadius: 0 }}
                onClick={() => {
                  setModalOpen(religion);
                  setActiveTab(0);
                }}
              >
                ...
                <div className="relative z-10 flex flex-col gap-4">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <div
                        className={`text-sm font-semibold mb-1 tracking-wide ${chivoMono} ${religionColors[religion].accent}`}
                      >
                        {religion}
                      </div>
                      {/* Featured quote at top, large */}
                      <div
                        className={`text-3xl mb-2 leading-snug ${isError ? "text-red-300" : "text-white"} ${chivoMono}`}
                      ...
                      {section.featured_quote_source &&
                        section.featured_quote_source.url && (
                          <a
                            ...
                            className={`block text-xs mt-1 mb-2 text-blue-300 underline ${chivoMono}`}
                    ...
                    {section.status !== null && (
                      <div
                        className={`text-xl font-semibold ${statusColors[section.status]} ${chivoMono} ${isError ? "animate-pulse" : ""}`}
                      >
                  ...
                  <div
                    className={`mb-4 leading-relaxed text-lg mt-2 ${instrumentSans}`}
                    ...
                    {Array.isArray(section.sources) &&
                      section.sources.map((src: Source, i: number) => (
                        <motion.a
                          ...
                          className={`block px-4 py-3 text-xs shadow hover:scale-105 hover:brightness-110 transition-all ${chivoMono}`}
                          style={{
                            borderRadius: 0,
        ...
        conclusion ? (
          <motion.div
            ...
            className={`bg-gray-900/80 border border-gray-700 p-10 shadow-lg mt-4 ${instrumentSans}`}
            style={{ borderRadius: 0 }}
          >
            <div
              className={`text-gray-200 leading-relaxed text-2xl font-semibold mb-4 ${chivoMono}`}
            >
              CONCLUSION
            </div>
            <div>
                          ...
                          <div
                            ...
                            dangerouslySetInnerHTML={{
                              __html: renderMarkdownWithReferences(
                                summary,
                                {
                                  judaism: sections.judaism.sources || [],
                                  christianity:
                                    sections.christianity.sources || [],
                                  islam: sections.islam.sources || [],
                                },
                                (rel) => getAccentColor(rel),
                              ),
                            }}
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: renderMarkdownWithReferences(
                          ...
                          {
                            judaism: sections.judaism.sources || [],
                            christianity: sections.christianity.sources || [],
                            islam: sections.islam.sources || [],
                          },
                          (rel) => getAccentColor(rel),
                        ),
